...
class Picker {
  pickerMode: PickerMode = PickerMode.SOLID;
  opacity: number = 1;
  color: number = 0x257AC8;
...
export default function ($) {
  var ColorPicker = function (element, options) {
    ...
    `);
    this.$colorModeList = $('<ul class="dropdown-menu" />').append(
    ...
    this.$colorPickerContainer
      ...
      .append(
        this.$slidersContainer
          .append(
            this.$hueSliderContainer.append(this.$hueSlider.slider({ value: this.hue, max: 360 }))
          )
          .append(
            this.$opacitySliderContainer
              .append(this.$opacitySliderContainerPatch)
              .append(
                this.$opacitySlider.slider({ value: this.options.opacity * 100 })
              )
          )
      )
      .append(
        this.$cutomColorSettings
          .append(
            this.$colorMode.append(this.$colorModeList)
          )
  ...
  ColorPicker.prototype.changeHue = function (value: number) {
    const newColor = Color().hsv(value, this.saturation, this.brightness);
    this.changeVal(newColor, this.options.opacity * 100);
  }
  ColorPicker.prototype.changeOpacity = function (value: number) {
  ...
  ColorPicker.prototype.position = function (x, y, event?) {
    ...
    if (!this.$saturationHandler.is('.hover')) draggable = true;
