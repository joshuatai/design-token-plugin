...
const usePropertyInfo = (property, fromTokenList = false) => {
  ...
  if (_property) {
    ...
    if (_property.type === PropertyTypes.OPACITY) {
      const applyThemeMode: ThemeMode = getThemeMode(_property.themeMode) as ThemeMode;
      ...
      if (_property.useToken) {
        const useToken = getToken(_property.useToken) as Token;
        value = useToken.name;
        secondValue = '';
    ...
    if (_property.type === PropertyTypes.FILL_COLOR) {
      const applyThemeMode: ThemeMode = fromTokenList ? currentMode : getThemeMode(_property.themeMode) as ThemeMode;
      if (themeModes.length > 1) {
        applyThemeMode ? thridValue = applyThemeMode.name : thridValue = defaultMode.name;
      }
