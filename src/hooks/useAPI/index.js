...
const useAPI = () => {
    ...
    const { setAPI, } = useContext(APISetterContext);
    const setLocalAPI = function ({ key, tid, vid, lastV, aid, admin }) {
        sendMessage(MessageTypes.SET_API, {
            'api-key': key,
            'tokens-id': tid,
            // 'collection-id': cid,
            'versions-id': vid,
    ...
    const connectAPI = (key, tokensID, password, admin) => __awaiter(void 0, void 0, void 0, function* () {
        const options = {
            method: 'GET',
            headers: {
                'secret-key': null
                    ...
                    if (password) {
                        return fetch(`${JSONBIN_URL}/b/${api.aid}`, options)
                            ...
                            if (res.password === password) {
                                api.admin = true;
                                setLocalAPI(api);
                                return Promise.resolve(Object.assign(Object.assign({}, api), { data, success: true }));
                            }
                            return Promise.reject({
                        ...
                        if (password === '')
                            api.admin = false;
                        if (admin !== undefined)
                            admin === true ? api.admin = true : api.admin = false;
                        if (api.admin) {
                            setLocalAPI(api);
                            return Promise.resolve(Object.assign(Object.assign({}, api), { data, success: true }));
                        }
                    ...
                    if (versions.success, admin.success) {
                        ...
                        return fetch(`${JSONBIN_URL}/b`, options)
                            ...
                            if (res.success) {
                                ...
                                api.admin = true;
                                setLocalAPI(api);
                                return Promise.resolve(Object.assign(Object.assign({}, api), { data: res.data.data, success: res.success }));
                            }
