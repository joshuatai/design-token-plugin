            ...
            const data = groups.map(({ id, name, tokens }) => {
                return {
                    ...
                    tokens: tokens.map(tokenId => {
                        const { name, propertyType, properties } = getToken(tokenId);
                        return {
                            name,
                            propertyType,
                            properties: properties.map(propId => {
                                const property = _cloneDeep(Object.assign({}, getProperty(propId)));
                                property.type = property._type;
                                delete property.parent;
                                delete property.id;
                                delete property._type;
                                return property.useToken ? getToken(property.useToken).name : property;
                            })
                        };
                    })
                };
