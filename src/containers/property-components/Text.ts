...
import FontSizeModel from 'model/Text';
...
export default function ($) {
  var FontSize = function (element, options) {
    const tokensMap = getPureToken(PropertyTypes.TEXT);
    let tokenList = Object.keys(tokensMap).map(key => tokensMap[key]);
    const useToken = getToken(options.useToken);
    hostData = this;
    this.options   = new FontSizeModel(options);
    ...
    this.$fontSizeIcon = PropertyIcon(this.options).$icon;
    this.$fontSizeValue = $('<span class="font-size-val"></span>').attr('contenteditable', !useToken);
    this.$detachToken = $(`
    ...
    this.$element
      .append(
        this.$customVal
          .append(
            this.$valContainer
              .append(this.$fontSizeIcon)
              .append(this.$fontSizeValue.text(useToken ? useToken.name : this.options.fontSize))
              .append(
                tokenList.length ?
  ...
  FontSize.prototype.useToken = function (token) {
    const { fontSize } = token.properties[0];
    this.options.useToken = token.id;
    this.options.fontSize = fontSize;
    this.$fontSizeValue.text(token.name).attr('contenteditable', false);
    this.$detachToken.data('token', token).show();
  }
  FontSize.prototype.detachToken = function (token) {
    const usedProperty = token.properties[0];
    this.options.useToken = '';
    this.$fontSizeValue.text(usedProperty.fontSize).attr('contenteditable', true);
    this.$detachToken.removeData('token').hide();
  }
  FontSize.prototype.destroy = function () {
  ...
  function Plugin(option) {
    return this.each(function () {
      ...
      if (!data) $this.data(NAME, (data = new FontSize(this, options)));
    })
  }
