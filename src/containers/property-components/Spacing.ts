...
export default function ($) {
        ...
        this.$customVal
          .append(
            this.$valContainer
              .append(this.$icon)
              .append(
                this.$spacingValue.text(spacingValue).attr('title', spacingValue).addClass(this.tokenList.length ? 'hasReferenceToken' : '')
              )
              .append(this.$token)
          )
      );
  ...
  function Plugin(option) {
    return this.each(function () {
      ...
      if (!data) $this.data(NAME, (data = new Spacing(this, options)));
    })
  }
