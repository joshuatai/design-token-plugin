...
const APISetting = ({
  ...
  return (
    <div className="api-setting-panel">
      <label>API Key</label>
      <div ref={apiKeyRef} className={`api-key ${processing ? 'disabled' : ''}`} prop-name="key" is-required="true" contentEditable="true" suppressContentEditableWarning={true}>{apiKey}</div>
      <label>Bin ID</label>
      <div ref={binIDRef} className={`bin-id ${processing ? 'disabled' : ''}`} prop-name="bin-id" contentEditable="false" suppressContentEditableWarning={true}>{tokensID}</div>
      <span className="text-muted">Enter an existing Bin ID or leave it blank to generate a new Bin.</span>
      <label>Admin Password</label>
      <div ref={adminPWDRef} className={`admin-pwd ${processing ? 'disabled' : ''}`} prop-name="password" contentEditable="false" suppressContentEditableWarning={true}></div>
      <span className="text-muted">Required for the first time setup or the administrator. For password, it is at least 8-16 characters.</span>
      <button className="btn btn-primary" onClick={settingHandler} disabled={processing || !savable}>
        {
